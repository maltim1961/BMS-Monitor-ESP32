...
// ИСПРАВЛЕНИЕ: НЕ инициализируем qBuffer здесь!
uint16_t* qBuffer = nullptr;  // Будет инициализирован в axs15231_init()
...
{
    // ИСПРАВЛЕНИЕ: Инициализируем qBuffer ЗДЕСЬ!
    if (qBuffer == nullptr) {
        Serial.println("Allocating qBuffer from PSRAM...");
        qBuffer = (uint16_t*) heap_caps_malloc(230400, MALLOC_CAP_SPIRAM);
        
        if (qBuffer == nullptr) {
            Serial.println("ERROR: Failed to allocate qBuffer from PSRAM!");
            Serial.println("Trying internal RAM...");
            qBuffer = (uint16_t*) heap_caps_malloc(230400, MALLOC_CAP_INTERNAL);
            
            if (qBuffer == nullptr) {
                Serial.println("FATAL: Cannot allocate qBuffer at all!");
                return;
            }
        }
        Serial.println("qBuffer allocated successfully!");
    }
...
void lcd_fill(uint16_t xsta, uint16_t ysta, uint16_t xend, uint16_t yend, uint16_t color)
{
    ...
    // ИЗМЕНЕНО: используем PSRAM вместо MALLOC_CAP_INTERNAL
    uint16_t *color_p = (uint16_t *)heap_caps_malloc(w * h * 2, MALLOC_CAP_SPIRAM);
    
    if (color_p == nullptr) {
        Serial.println("ERROR: Cannot allocate memory for lcd_fill!");
        return;
    }
...
void lcd_PushColors_rotated_90(
                    ...
                    uint16_t *data)
{
    // ИСПРАВЛЕНИЕ: Проверяем qBuffer перед использованием!
    if (qBuffer == nullptr) {
        Serial.println("ERROR: qBuffer is NULL in lcd_PushColors_rotated_90!");
        return;
    }
